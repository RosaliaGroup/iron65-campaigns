...
function jshowBookingBtn(){
  var m=document.getElementById('jmsgs');
  var d=document.createElement('div');
  d.innerHTML='<a href="'+BOOKING_URL+'" target="_blank" style="display:block;text-align:center;background:#C8A96E;color:#000;padding:12px;font-size:11px;font-weight:700;letter-spacing:2px;text-decoration:none;border-radius:6px;margin:8px 2px;font-family:Montserrat,sans-serif;text-transform:uppercase">\uD83D\uDCCB Book My Tour \u2192</a>';
...
function jpclick(l){
  ...
  if(l.indexOf('Book a Tour')!==-1||l.indexOf('Ready')!==-1){
    jbot('Perfect! Our booking form takes about 60 seconds \u2014 you\u2019ll pick your date, time, and share a few quick details there. Ana will confirm by text within the hour! \uD83D\uDCCB');
    setTimeout(jshowBookingBtn,300);
...
async function japi(msg){
  ...
  try{
    ...
    if(reply.toLowerCase().match(/book|tour|visit|schedule/))setTimeout(function(){jpill(["\uD83D\uDCC5 Book a Tour"])},500);
  }catch(e){
    var el=document.getElementById('jtyp');if(el)el.remove();
